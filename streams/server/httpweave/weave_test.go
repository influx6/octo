...
import (
	...
	"github.com/influx6/octo/streams/server/httpweave"
...
func TestHTTPWeaveProtocol(t *testing.T) {
	...
	t.Logf("\tWhen request %q for a tcp resource through http", consts.ContactRequest)
	{
		...
		req, err := newMessageRequest(header, commando.MakeByteMessage(consts.ContactRequest))
		if err != nil {
...
func newWeaveHTTP(t *testing.T, inst octo.Instrumentation, authenticate octo.Authenticator, cred octo.Credentials, system server.System) *httpweave.WeaveServerMux {
